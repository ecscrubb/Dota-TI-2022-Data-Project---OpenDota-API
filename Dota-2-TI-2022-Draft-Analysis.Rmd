...
This plot clearly supports the hypotheses outlined above. All but one of the supports among the top-25 most-contested heroes (Snapfire, Silencer, Crystal Maiden, Tiny, Clockwerk, Nyx Assassin, Undying, and Marcy) fall to the right of the dashed line. The only "support" to the left is Primal Beast, who is often played as a core and only sometimes as a support. The majority of these supports also sport a low average draft position, with most being drafted before the 9th pick/ban (Snapfire is the only exception). Meanwhile, *all* the heroes to the left of the dashed line are cores, and most of them averaged a draft position later than the 9th pick/ban. In The International 2022, teams showed a strong propensity to *ban* cores early and *pick* them late, preferring to spend their early picks on supports.
...
all_drafts %>%
  ...
  ggplot(mapping = aes(y = reorder(hero_name, appearances))) +
    geom_bar(mapping = aes(x = appearances), stat = "identity"
             , fill = "lightblue2") +
    geom_bar(mapping = aes(x = wins), stat = "identity"
             , fill = "slateblue4") +
    geom_text(mapping = aes(label = appearances, x = appearances
                            , hjust = 1.5, vjust = 0.3)
              , size = 3.5
              ) +
    geom_text(mapping = aes(label = paste0(wins, " (", percent(win_rate, 0.1)
                                           , ")"
                                           )
                            , x = wins, hjust = 1.1, vjust = 0.3
...
This chart indicates that pros' assessment of these heroes was a bit of a mixed bag. In a broad sense, it seems pros were generally right about the overall strength of these heroes, as most sport a 50\%\+ win rate - impressive for cores picked early in the draft with so many opportunities left for the opposing team to counter them. On the other hand, pros' tendency to ban Primal Beast in the early phases while letting Leshrac through seems to have been a mistake. Primal Beast was banned 125 times but ended the tournament with only a 47.76\% win rate. Meanwhile, Leshrac was banned 72 times and allowed to be drafted 77 times, and ended the tournament with a 57.14\% win rate.

Of course, these are just broad, overall trends, which by themselves can't give us a well-rounded view of the tournament as a whole. More interesting insights will require us to do more fine-grained analysis. Two analytical paths seems especially interesting to me. One is examining the relationship of *draft phases* to heroes' win rate. The other is to identify pros' evolving understanding of and adaptation to the "meta" over the course of the tournament by examining *trends over time.* I will turn to these questions in the coming sections.
